...
static	const	unsigned int	paddingSize            = 6; // An extra 8K per allocation!
#else
static	const	unsigned int	hashBits               = 12;
static		bool		randomWipe             = false;		//false
static		bool		alwaysValidateAll      = false;		//false	- really slows down everything :(
static		bool		alwaysLogAll           = false;		//false
static		bool		alwaysWipeAll          = true;
static		bool		cleanupLogOnFirstRun   = true;
static	const	unsigned int	paddingSize            = 4;
#endif

void MMGRStartStressTest()
{
	randomWipe             = true;
...
#ifdef USE_INTERNAL_MEM_LEAK_TRACKER
...
static	void	dumpAllocations(FILE *fp)
{
	...
	for (unsigned int i = 0; i < hashSize; i++)
	{
		...
		while(ptr)
		{
//!!!added by zack to not show non fixable leaks
if( ptr->sourceLine != 0 )
