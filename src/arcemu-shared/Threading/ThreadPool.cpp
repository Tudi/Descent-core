...
uint32 CThreadPool::GetLastThreadUpdateStamp()
{
	uint32 min_stamp = 0xFFFFFFFF;
	_mutex.Acquire();
	for(ThreadSet::iterator itr = m_activeThreads.begin(); itr != m_activeThreads.end(); ++itr)
	{
		if((*itr)->ExecutionTarget)
		{
			uint32 this_stamp = (*itr)->ExecutionTarget->last_updated;
...
void CThreadPool::Startup()
{
	...
	Log.Debug("ThreadPool", "Startup, launched %u threads.", i);
}
...
static void * thread_proc(void * param)
{
	...
	// return to let the stack unwind properly
	return NULL;
}
...
Thread * CThreadPool::StartThread(ThreadBase * ExecutionTarget)
{
	...
	t->SetupMutex.Acquire();
	{
		...
		if (res = pthread_create(&target, NULL, &thread_proc, (void*)t),
			!(res == 0))
		{
			Log.Error("ThreadPool", "Failed to create thread. (Error: %d)", res);
			t->SetupMutex.Release();
			delete t;
			_mutex.Release();
			return NULL;
