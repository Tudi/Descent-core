...
void Database::StartQueryLogging(char *filename)
{
	if( QueryLogs )
		fclose( QueryLogs );
	QueryLogs = fopen( filename, "a" );
}
...
bool Database::Execute(const char* QueryString, ...)
{
	...
	va_end(vlist);
	if( QueryLogs && query[0] != 's' && query[0] != 'S' )
	{
		fprintf(QueryLogs, "%s;\n", query);
		fflush(QueryLogs);
	}
...
bool Database::run()
{
	...
	while(1)
	{
		...
		if (query == NULL)
			Sleep(2);	//hmm, if too large then produces lag, if too small it eats up CPU 
	}
