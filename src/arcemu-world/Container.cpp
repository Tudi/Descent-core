...
Container::~Container( )
{
	INSTRUMENT_TYPECAST_CHECK_CONTAINER_OBJECT_TYPE
   for(uint32 i = 0; i < m_itemProto->ContainerSlots; i++)
	{
		if( m_Slot[i] != NULL )
		{
			//wtf fix is this ?
			if( m_Slot[i]->GetOwner() != m_owner )
				continue;
			m_Slot[i]->DeleteMe();
			m_Slot[i] = NULL;
		}
...
//	for( uint32 i=0;i<m_itemProto->ContainerSlots;i++)
	for( uint32 i=0;i<MAX_CONTAINER_SLOTS;i++)
		m_Slot[i] = NULL;
}
...
int16 Container::FindFreeSlot()
{
	INSTRUMENT_TYPECAST_CHECK_CONTAINER_OBJECT_TYPE
	int16 TotalSlots = (int16)(GetUInt32Value( CONTAINER_FIELD_NUM_SLOTS ));
...
bool Container::AddItem(uint16 slot, Item *item)
{
	INSTRUMENT_TYPECAST_CHECK_CONTAINER_OBJECT_TYPE
	...
	if(m_owner->IsInWorld() && !item->IsInWorld())
	{
		...
		m_owner->PushCreationData( &buf, count, item );
	}
	return true;
}
...
void Container::SwapItems(int16 SrcSlot, int16 DstSlot)
{
	INSTRUMENT_TYPECAST_CHECK_CONTAINER_OBJECT_TYPE
	Item *temp;
	if( SrcSlot < 0 || SrcSlot >= (int16)m_itemProto->ContainerSlots )
	{ 
		return;
	}
	
	if( DstSlot < 0 || DstSlot >= (int16)m_itemProto->ContainerSlots )
	{ 
		return;
	}
...
bool Container::SafeFullRemoveItemFromSlot(int16 slot)
{
	...
	if( slot >= (int32)GetProto()->ContainerSlots )
		return false;
	...
	for(slot = 0; slot < GetProto()->ContainerSlots; slot++)
	{
		if(!m_Slot[slot])
		{
			...
			if(m_owner->IsInWorld() && !pItem->IsInWorld())
			{
				...
				m_owner->PushCreationData( &buf, count, pItem );
			}
			if(r_slot)
				*r_slot = slot;
			return true;
...
{
	INSTRUMENT_TYPECAST_CHECK_CONTAINER_OBJECT_TYPE
	(SafeItemCast(this))->SaveToDB(INVENTORY_SLOT_NOT_SET, slot, first, buf);

	for(uint32 i = 0; i < m_itemProto->ContainerSlots; i++)
		if (m_Slot[i] 
//			&& !((m_Slot[i]->GetProto()->Flags)& ITEM_FLAG_CONJURED)	//after 15 minutes of logout they expire not always. DC much ?
				)
