...
bool Transporter::GenerateWaypoints()
{
	...
	for (size_t i = 0; i < keyFrames.size() - 1; i++)	   //
	{
		...
		TWayPoint pos(keyFrames[i + 1].mapid, keyFrames[i + 1].x, keyFrames[i + 1].y, keyFrames[i + 1].z, teleport2);

		//		sLog.outString("T: %d, x: %f, y: %f, z: %f, t:%d", t, pos.x, pos.y, pos.z, teleport2);
...
Transporter::~Transporter()
{
	...
	for(TransportNPCMap::iterator itr = m_npcs.begin(); itr != m_npcs.end(); ++itr)
	{
		...
		{
			delete SafeCreatureCast( itr->second )->m_transportPosition;
			SafeCreatureCast( itr->second )->m_transportPosition = NULL;
		}
...
Creature * Transporter::GetCreature(uint32 Guid)
{
	...
	if(itr==m_npcs.end())
		return NULL;
	if(itr->second->GetTypeId()==TYPEID_UNIT)
		return SafeCreatureCast( itr->second );
	else
		return NULL;
}
