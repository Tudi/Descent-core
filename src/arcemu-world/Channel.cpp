...
void Channel::AttemptJoin(Player * plr, const char * password,bool is_zone_swap)
{
	Guard mGuard(m_lock);
	...
	if( 
		...
		plr->GetSession()->CanUseCommand('k') 
#else
		(( !m_general && plr->GetSession()->CanUseCommand('c') ) || plr->GetSession()->CanUseCommand('a') )
#endif
		)
	...
	if(m_members.find(plr) != m_members.end()
		&& is_zone_swap == false //make client show correct animation :P
		)
	{
	...
	//every GM is considered channel owner
	if( m_GMmoderated == true && plr->GetSession()->HasPermissions() )
		flags |= CHANNEL_FLAG_OWNER;
...
void Channel::Part(Player * plr,bool quet)
{
	m_lock.Acquire();
	WorldPacket data( SMSG_CHANNEL_NOTIFY, 200);
	uint32 flags;
	...
	else if( quet == false && plr->GetSession() )
	{
...
void Channel::SetOwner(Player * oldpl, Player * plr)
{
	...
	WorldPacket data( SMSG_CHANNEL_NOTIFY, 500);
	if(oldpl != NULL)
	{
	...
	else
	{
		for(MemberMap::iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
		{
			...
			{
				if( plr == itr->first && ( !m_general || (m_GMmoderated == true && plr->GetSession()->HasPermissions()) ) )
				{
					pOwner = itr->first;
					oldflags = itr->second;
					itr->second |= CHANNEL_FLAG_OWNER;
	...
	if(pOwner == NULL)
		return;		// obviously no members or owner candidates
...
void Channel::Invite(Player * plr, Player * new_player)
{
	...
	sStackWorldPacket( data,SMSG_CHANNEL_NOTIFY, 500);
...
void Channel::Moderate(Player * plr)
{
	...
	if(!(itr->second & CHANNEL_FLAG_OWNER || itr->second & CHANNEL_FLAG_MODERATOR) 
		...
		&& !plr->GetSession()->CanUseCommand('k') 
#else
		&& !plr->GetSession()->CanUseCommand('c') 
#endif
		)
	{
...
void Channel::Say(Player * plr, const char * message, Player * for_gm_client, bool forced)
{
	...
	if(!forced)
	{
		...
		if(m_muted && !(itr->second & CHANNEL_FLAG_VOICED)
			&& !(itr->second & ( CHANNEL_FLAG_OWNER | CHANNEL_FLAG_MODERATOR)) 
#ifdef FORCED_GM_TRAINEE_MODE
		&& !plr->GetSession()->CanUseCommand('k') 
#else
		&& !plr->GetSession()->CanUseCommand('a') 
#endif
			)
		{
			data << uint8(CHANNEL_NOTIFY_FLAG_YOUCANTSPEAK) << m_name;
	...
	if( plr->bGMTagOn && for_gm_client == NULL && plr->getGender() )
	{
...
#endif
		new_msg += message;
		uint32 messageLength = (uint32)new_msg.length()+1;
		data << messageLength;
		data << new_msg;
//		data << (uint8)(plr->bGMTagOn ? CHAT_FLAG_GM_TAG : 0);
		data << (uint8)(0);
	}
	else
	{
...
{
	sStackWorldPacket( data, SMSG_CHANNEL_NOTIFY, 100);
	data << uint8(CHANNEL_NOTIFY_FLAG_NOTON) << m_name;
	plr->GetSession()->SendPacket(&data);
}
...
void Channel::Kick(Player * plr, Player * die_player, bool ban)
{
	...
	if(!(me_itr->second & ( CHANNEL_FLAG_OWNER | CHANNEL_FLAG_MODERATOR)) 
#ifdef FORCED_GM_TRAINEE_MODE
		&& !plr->GetSession()->CanUseCommand('k') 
#else
		&& !plr->GetSession()->CanUseCommand('a') 
...
void Channel::Unban(Player * plr, PlayerInfo * bplr)
{
	...
	if(!(itr->second & ( CHANNEL_FLAG_OWNER | CHANNEL_FLAG_MODERATOR)) 
#ifdef FORCED_GM_TRAINEE_MODE
		&& !plr->GetSession()->CanUseCommand('k') 
#else
		&& !plr->GetSession()->CanUseCommand('a') 
...
{
	Guard mGuard(m_lock);
	sStackWorldPacket( data, SMSG_CHANNEL_LIST, 50 + 5500 + 100 );
	MemberMap::iterator itr = m_members.find(plr);
	if(itr == m_members.end())
	{
		data.Initialize(SMSG_CHANNEL_NOTIFY);
...
bool ChannelMgr::IsValidChannelName( const char *Name )
{
	for( uint32 i=0; i<200 && Name[i] != 0; i++ )
	{
		if( Name[i] == 0 )
			return true;
		//no formatting. Ex : inserting picture links or colors
		if( Name[i] == '|' )
			return false;
	}
	return true;
