...
EyeOfTheStorm::EyeOfTheStorm(MapMgr * mgr, uint32 id, uint32 lgroup, uint32 t) : CBattleground(mgr,id,lgroup,t)
{
	...
	m_standFlag = m_dropFlag = NULL;
	m_bubbles[0] = m_bubbles[1] = NULL;
}
...
EyeOfTheStorm::~EyeOfTheStorm()
{
	if(m_standFlag != NULL)
	{
		...
		{
			sGarbageCollection.AddObject( m_standFlag );
			m_standFlag = NULL;
		}
	}
	...
	if(m_dropFlag != NULL)
	{
		...
		{
			sGarbageCollection.AddObject( m_dropFlag );
			m_dropFlag = NULL;
		}
	}
	...
	for(uint32 i = 0; i < EOTS_TOWER_COUNT; ++i)
	{
		if(m_CPStatusGO[i] != NULL)
		{
			...
			{
				sGarbageCollection.AddObject( m_CPStatusGO[i] );
				m_CPStatusGO[i] = NULL;
			}
		}
		if(m_CPBanner[i] != NULL)
		{
			...
			{
				sGarbageCollection.AddObject( m_CPBanner[i] );
				m_CPBanner[i] = NULL;
			}
		}
		if(m_CPBanner2[i] != NULL)
		{
			...
			{
				sGarbageCollection.AddObject( m_CPBanner2[i] );
				m_CPBanner2[i] = NULL;
			}
		}
		if(m_CPBanner3[i] != NULL)
		{
			...
			{
				sGarbageCollection.AddObject( m_CPBanner3[i] );
				m_CPBanner3[i] = NULL;
			}
		}
		if(EOTSm_buffs[i] != NULL)
		{
			...
			{
				sGarbageCollection.AddObject( EOTSm_buffs[i] );
				EOTSm_buffs[i] = NULL;
			}
		}
	...
	for(uint32 i = 0; i < 2; ++i )
	{
		if(m_bubbles[i] != NULL)
		{
			...
			{
				sGarbageCollection.AddObject( m_bubbles[i] );
				m_bubbles[i] = NULL;
			}
		}
	}
...
void EyeOfTheStorm::HookOnAreaTrigger(Player * plr, uint32 id)
{
	...
	*/
	...
	plr->RemoveAura( EOTS_NETHERWING_FLAG_SPELL,0,AURA_SEARCH_POSITIVE );
...
{
	uint32 i;
	InRangePlayerSetRecProt::iterator itr, itrend;
	Player * plr;
	GameObject * go;
	int32 delta = 0;
	...
	for(i = 0; i < EOTS_TOWER_COUNT; ++i)
	{
		...
		itr = go->GetInRangePlayerSetBegin( AutoLock );
		itrend = go->GetInRangePlayerSetEnd();
		for( ; itr != itrend; ++itr )
		{
			plr = *itr;
			if( plr->isAlive() && !(plr->IsStealth()) && !(plr->IsInvisible()) && !(plr->SchoolImmunityAntiEnemy[0]) && plr->GetDistance2dSq( go ) <= EOTS_CAPTURE_DISTANCE )
			{
...
void EyeOfTheStorm::SpawnBuff(uint32 x)
{
	...
	if(EOTSm_buffs[x] == NULL)
	{
		...
		EOTSm_buffs[x]->SetFloatValue(GAMEOBJECT_PARENTROTATION_2, EOTSBuffRotations[x][0]);
		EOTSm_buffs[x]->SetFloatValue(GAMEOBJECT_PARENTROTATION_3, EOTSBuffRotations[x][1]);
