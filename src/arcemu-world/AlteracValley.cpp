...
//	m_boss = NULL;
	...
	if( g_HomeNpcInfo[m_nodeId].id_a != 0 )
	{
		...
		{
			Creature *p_homeNPC = m_bg.SpawnCreature(g_HomeNpcInfo[m_nodeId].id_a, g_HomeNpcInfo[m_nodeId].a_x, g_HomeNpcInfo[m_nodeId].a_y,
				g_HomeNpcInfo[m_nodeId].a_z, g_HomeNpcInfo[m_nodeId].a_o);
			if( p_homeNPC )
				m_homeNPC = p_homeNPC->GetGUID();
		}
		else
		{
			Creature *p_homeNPC = m_bg.SpawnCreature(g_HomeNpcInfo[m_nodeId].id_h, g_HomeNpcInfo[m_nodeId].h_x, g_HomeNpcInfo[m_nodeId].h_y,
...
AVNode::~AVNode()
{
	...
	for(vector<uint64>::iterator itr = m_guards.begin(); itr != m_guards.end(); ++itr)
	{
		Creature *c = m_bg.GetMapMgr()->GetCreature( (*itr) );
		if( c )
			c->Despawn(0, 0);
	}
	m_guards.clear();
	Creature *p_homeNPC = m_bg.GetMapMgr()->GetCreature( m_homeNPC );
	if( p_homeNPC != NULL )
	{
		p_homeNPC->Despawn(0, 0);
		m_homeNPC = 0;	//despawn without respawn = delete
	}
	if( m_spiritGuide )
	{
...
#endif*/
				z = m_bg.GetMapMgr()->GetLandHeight(x, y);
				if( z != WMO_VMAP_VALUE_NOT_INITIALIZED )
				{
					Creature *c = m_bg.SpawnCreature(m_template->m_guardId[t], x, y, z, 0.0f);
					if( c )
						m_guards.push_back( c->GetGUID() );
				}
			}
		}
	}
...
void AVNode::Capture()
{
	...
	if( !m_template->m_capturable )
	{
		if( m_state != m_template->m_defaultState )
		{
			...
			if( m_template->m_defaultState == AV_NODE_STATE_ALLIANCE_CONTROLLED )
			{
				for(InRangePlayerSet::iterator itx = m_bg.m_players[1].begin(); itx != m_bg.m_players[1].end(); ++itx)
				{
					Player * plr = (*itx);
					if(!plr) continue;
			...
			{
				for(InRangePlayerSet::iterator itx = m_bg.m_players[0].begin(); itx != m_bg.m_players[0].end(); ++itx)
				{
					Player * plr = (*itx);
					if(!plr) continue;
...
bool AlteracValley::HookHandleRepop(Player * plr)
{
	...
	// port to it
	plr->SafeTeleportDelayed(plr->GetMapId(), plr->GetInstanceID(), dest_pos);
	return false;
}
...
void AlteracValley::OnCreate()
{
	...
	gate->SetFloatValue(GAMEOBJECT_PARENTROTATION_2,0.0129570f);
	gate->SetFloatValue(GAMEOBJECT_PARENTROTATION_3,-0.0602880f);
	...
	gate->SetFloatValue(GAMEOBJECT_PARENTROTATION_2, 0.36f);
	gate->SetFloatValue(GAMEOBJECT_PARENTROTATION_3, 0.922766f);
	gate->PushToWorld(m_mapMgr);
	m_gates.push_back(gate);
...
void AlteracValley::HookOnUnitKill(Player * plr, Unit * pVictim)
{
	...
	if(pVictim->GetEntry() == AV_NPC_GENERAL_VANNDAR_STORMPIKE) 
	{
		...
		for(InRangePlayerSet::iterator itx = m_players[1].begin(); itx != m_players[1].end(); ++itx)
		{
			Player * plr = (*itx);
			if(!plr) continue;
	...
	else if(pVictim->GetEntry() == AV_NPC_GENERAL_DREK_THAR)
	{
		...
		for(InRangePlayerSet::iterator itx = m_players[0].begin(); itx != m_players[0].end(); ++itx)
		{
			Player * plr = (*itx);
			if(!plr) continue;
...
void AlteracValley::HookGenerateLoot(Player *plr, Object *pCorpse)
{
	...
	while(loot_ptr->ItemId != 0)
	{
		...
		{
			if( RandChance(loot_ptr->Chance * sWorld.getRate(RATE_DROP0)) )
			{
				__LootItem li;
				...
				if( pProto != NULL )
				{
					...
					// push to vector
					SafeCorpseCast(pCorpse)->loot.items.push_back(li);
				}
			}
		}
	...
	// set it
	SafeCorpseCast(pCorpse)->loot.gold = float2int32(gold);
}
