...
{
    LocationVector dest;
	//sprit guide or graveyard locations
	if(plr->GetTeam())
		dest.ChangeCoords(1056.0f, 1373.0f, 329.0f, 0.0f);
	else
		dest.ChangeCoords(1406.0f, 1563.0f, 329.0f, 3.0f);
...
void WarsongGulch::PeriodicUpdateClientRemainingTime()
{
...
void WarsongGulch::OnCreate()
{
	...
	GameObject *gate;
	// Alliance Gates
//	gate = SpawnGameObject(179921, 489, 1471.554688f, 1458.778076f, 362.633240f, 0, GAMEOBJECT_CLICKABLE|GAMEOBJECT_UNCLICKABLE, 114, 2.33271f);
//	gate->SetUInt32Value(GAMEOBJECT_DYNAMIC, 4294901760);
//	gate->PushToWorld(m_mapMgr);
//	m_gates.push_back(gate);

	gate = SpawnGameObject(179919, 489, 1492.477783f, 1457.912354f, 342.968933f, 0, GAMEOBJECT_CLICKABLE|GAMEOBJECT_UNCLICKABLE, 114, 2.68149f);
	gate->SetUInt32Value(GAMEOBJECT_DYNAMIC, 4294901760);
	gate->PushToWorld(m_mapMgr);
	m_gates.push_back(gate);
	//ghost gate, or else aliance can walk trough the gate
	gate = SpawnGameObject(180322, 489, 1492.477783f, 1457.912354f, 342.968933f, 0, GAMEOBJECT_CLICKABLE, 114, 2.68149f);
	gate->SetUInt32Value(GAMEOBJECT_DYNAMIC, 4294901760);
	gate->PushToWorld(m_mapMgr);
	m_gates.push_back(gate);

	gate = SpawnGameObject(179918, 489, 1503.335327f, 1493.465820f, 352.188843f, 0, GAMEOBJECT_CLICKABLE|GAMEOBJECT_UNCLICKABLE, 114, 2.26f);
	gate->SetUInt32Value(GAMEOBJECT_DYNAMIC, 4294901760);
	gate->PushToWorld(m_mapMgr);
	m_gates.push_back(gate);
	//ghost gate, or else aliance can walk trough the gate
	gate = SpawnGameObject(180322, 489, 1503.335327f, 1493.465820f, 352.188843f, 0, GAMEOBJECT_CLICKABLE, 114, 2.26f);
	gate->SetUInt32Value(GAMEOBJECT_DYNAMIC, 4294901760);
	gate->PushToWorld(m_mapMgr);
	m_gates.push_back(gate);

	// Horde Gates
	gate = SpawnGameObject(179916, 489, 949.1663208f, 1423.7717285f, 345.6241455f, 5.7075057029724f, GAMEOBJECT_CLICKABLE|GAMEOBJECT_UNCLICKABLE, 114, 0.900901f);
	gate->SetFloatValue(GAMEOBJECT_PARENTROTATION,0);
	gate->SetFloatValue(GAMEOBJECT_PARENTROTATION_1,0);
	gate->SetFloatValue(GAMEOBJECT_PARENTROTATION_2,-0.707107f);
	gate->SetFloatValue(GAMEOBJECT_PARENTROTATION_3,0.707107f);
	gate->SetUInt32Value(GAMEOBJECT_DYNAMIC, 4294901760);
	gate->m_rotationX = (uint64)(0xFDDBAFFD769B74DA);
	gate->PushToWorld(m_mapMgr);
	m_gates.push_back(gate);
//	gate = SpawnGameObject(180322, 489, 949.1663208f, 1423.7717285f, 345.6241455f, 0.0f, GAMEOBJECT_CLICKABLE, 0, 0.900901f);
//	gate->SetFloatValue(GAMEOBJECT_PARENTROTATION_3,1.0f);
//	gate->SetUInt32Value(GAMEOBJECT_DYNAMIC, 4294901760);
//	gate->PushToWorld(m_mapMgr);
//	m_gates.push_back(gate);

	gate = SpawnGameObject(179917, 489, 953.0507202f, 1459.8424072f, 340.6525573f, 4.2865695953369f, GAMEOBJECT_CLICKABLE|GAMEOBJECT_UNCLICKABLE, 114, 0.854700f);   
	gate->SetFloatValue(GAMEOBJECT_PARENTROTATION,0);
	gate->SetFloatValue(GAMEOBJECT_PARENTROTATION_1,0);
	gate->SetFloatValue(GAMEOBJECT_PARENTROTATION_2,-0.707107f);
	gate->SetFloatValue(GAMEOBJECT_PARENTROTATION_3,0.707107f);
	gate->SetUInt32Value(GAMEOBJECT_DYNAMIC, 4294901760);
	gate->m_rotationX = (uint64)(0xE6C2BC50A512D11C);
	gate->PushToWorld(m_mapMgr);
	m_gates.push_back(gate);
//	gate = SpawnGameObject(180322, 489, 950.795166f, 1459.583374f, 342.152283f, 0.052358884364367f, GAMEOBJECT_CLICKABLE, 0, 0.854700f);   
//	gate->SetFloatValue(GAMEOBJECT_PARENTROTATION_3,1.0f);
//	gate->SetUInt32Value(GAMEOBJECT_DYNAMIC, 4294901760);
//	gate->PushToWorld(m_mapMgr);
//	m_gates.push_back(gate);
	...
	sEventMgr.AddEvent( this, &WarsongGulch::PeriodicUpdateClientRemainingTime, EVENT_BATTLEGROUND_REMAINING_TIME, 60*1000, 0,0);
	...
	// spawn spirit guides 
	AddSpiritGuide(SpawnSpiritGuide(1406.0f, 1563.0f, 329.0f, 3.0f, 0));
	AddSpiritGuide(SpawnSpiritGuide(1056.0f, 1373.0f, 329.0f, 0.0f, 1));
}

void WarsongGulch::OnStart()
{
	int32 TeamFactions[2] = { -1, -1 };
	for(uint32 i = 0; i < 2; ++i) 
	{
		for(InRangePlayerSet::iterator itr = m_players[i].begin(); itr != m_players[i].end(); ++itr) 
		{
			...
			if( plr->m_bg != this )
				continue;
			if( TeamFactions[ plr->m_bgTeam ] == -1 )
				TeamFactions[ plr->m_bgTeam ] = plr->GetTeamInitial(); //0 is horde
			//we actually joined the BG
			BattlegroundManager.SendBattlefieldStatus(plr, BF_STATUS_PACKET_PLAYER_JOINED, GetType(), GetMapMgr()->GetInstanceID(), (uint32)UNIXTIME - m_startTime, GetMapMgr()->GetMapId(), Rated() );
		}
	}
	// update pvp data
//	UpdatePvPData();
...
//		(*itr)->Despawn(5000, 0);
		(*itr)->Despawn(500, 0);	//horde can go out instantly
	}
	...
	if( m_SkipRewards )
	{
		//if team 1 is horde then set faction of the flag 2 to Aliance
		if( TeamFactions[0] == 0 )
			m_homeFlags[0]->SetFaction( 210 );
		else
			m_homeFlags[0]->SetFaction( 1314 );

		if( TeamFactions[1] == 0 )
			m_homeFlags[1]->SetFaction( 210 );
		else
			m_homeFlags[1]->SetFaction( 1314 );
