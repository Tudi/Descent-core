...
StrandOfTheAncient::~StrandOfTheAncient()
{
	...
	{
		sGarbageCollection.AddObject( m_relic );
		m_relic = 0;
	}
	...
	for (int i = 0; i < GATE_COUNT; i++)
	{
		...
		{
			sGarbageCollection.AddObject( m_gates[i] );
			m_gates[i] = NULL;
		}
		if (m_gateSigils[i] && !m_gateSigils[i]->IsInWorld())
		{
			sGarbageCollection.AddObject( m_gateSigils[i] );
			m_gateSigils[i] = NULL;
		}
		if (m_gateTransporters[i] && !m_gateTransporters[i]->IsInWorld())
		{
			sGarbageCollection.AddObject( m_gateTransporters[i] );
	...
	{
		sGarbageCollection.AddObject( m_endgate );
		m_endgate = NULL;
	}
