...
Map::Map(uint32 mapid, MapInfo * inf)
{
	...
	// Setup terrain
	sTerrainMgr.EventMapCreated( mapid );
	...
	for(uint32 x=0;x<_sizeX;x++)
	{
		if(spawns[x])
		{
			for(uint32 y=0;y<_sizeY;y++)
			{
				if(spawns[x][y])
				{	
					CellSpawnsVect * sp=spawns[x][y];
					sp->CreatureSpawns.DestroyContent();
					sp->GOSpawns.DestroyContent();
					delete sp;
					spawns[x][y]=NULL;
				}
			}
...
//					cspawn->proto = p;
					...
					{
						spawns[cellx]=new CellSpawnsVect*[_sizeY];
						memset(spawns[cellx],0,sizeof(CellSpawnsVect*)*_sizeY);
					}
					...
					cspawn->stand_state = fields[15].GetUInt16();
					cspawn->MountedDisplayID = fields[16].GetUInt32();
					cspawn->Item1SlotDisplay = fields[17].GetUInt32();
					cspawn->Item2SlotDisplay = fields[18].GetUInt32();
					cspawn->Item3SlotDisplay = fields[19].GetUInt32();
					cspawn->difficulty_spawnmask = fields[20].GetUInt32();
					cspawn->phase = fields[21].GetUInt32();
...
//				gspawn->o3=fields[10].GetFloat();
				...
				gspawn->precise_facing = (((uint64)fields[16].GetUInt32())) | (((uint64)fields[16].GetUInt32()) << 32 );
		...
		if(result)
		{
			if( CheckResultLengthGameObject(result) )
				...
				do{
					...
					gspawn->z=fields[5].GetFloat();
					gspawn->o=fields[6].GetFloat();
					gspawn->parent_rot1 =fields[7].GetFloat();
					gspawn->parent_rot2 =fields[8].GetFloat();
					gspawn->parent_rot3 =fields[9].GetFloat();
					gspawn->parent_rot4 =fields[10].GetFloat();
					gspawn->state=fields[11].GetUInt32();
					gspawn->flags=fields[12].GetUInt32();
					gspawn->faction=fields[13].GetUInt32();
					gspawn->scale = fields[14].GetFloat();
