		...
		else
		{
			...
			{	
				...
				else
				{
					...
					{
						uint64 tguid = guid;
						Creature *pCreature = plr->GetMapMgr()->GetCreature( tguid );
						if(pCreature)
						{
...
{
	// Maybe we should rewrite a function that generate the xyz after we figured if it's a creature or an gameobject.
	if(!*args) return false;
	...
	// Let's check if it's an gameobject
	else
	{
	...
	{
		GameObjectInfo *gameobjectResult = GameObjectNameStorage.LookupEntry(atol(starterId.c_str()));

		if(gameobjectResult)
			starterName = gameobjectResult->Name;
		else
		{
			recout = "|cff00ccffNo quest starter info found.\n\n";
			SendMultilineMessage(m_session, recout.c_str());
	...
	string tableName = (isCreature) ? ("creature_spawns") : ("gameobject_spawns");
	my_query = "SELECT map, position_x, position_y, position_z FROM " + tableName + " WHERE entry = " + starterId;
...
bool ChatHandler::HandleQuestRewardCommand(const char * args, WorldSession * m_session)
{
	...
	if(q)
	{
		for( int32 r = 0; r < q->count_reward_item; r++ )
		{
		...
		for( int32 r = 0; r < q->count_reward_choiceitem; r++ )
		{
