...
bool ChatHandler::HandleGMTicketListEventTelCommand(const char* args, WorldSession *m_session)
{
	if(!*args)
	{
INVALID_CALL_EARLY_EXIT:
		m_session->GetPlayer()->BroadcastMessage( "Command usage : .gmTicket getEvent <keyword> [0|1(dete ticket)] [0|1(teleport)]" );
		m_session->GetPlayer()->BroadcastMessage( "help : <keyword> may not contain non letter characters and cannot be empty. Default ticket does not get deleted and characters get teleported to the GM. Works only on online characters" );
		return false;
	}

	const char *keyword = args;
	char *del_start = (char *)strchr(args, ' ');
	if( del_start )
		*del_start = 0;	//mark end of the string
	string safe_keyword = CharacterDatabase.EscapeString( string(keyword) );
	if( strlen( safe_keyword.c_str() ) == 0 )
		goto INVALID_CALL_EARLY_EXIT;

	bool del_ticket = 0;
	if( del_start && atol(del_start+1) == 1 )
		del_ticket = 1;

	bool teleport = 1;
	if( del_start )
	{
		const char *tel_start = strchr(del_start+1, ' ');
		if( tel_start && atol(tel_start+1)==0 )
			teleport = 0;
	}
	...
	for(GmTicketList::iterator itr = objmgr.GM_TicketList.begin(); itr != objmgr.GM_TicketList.end(); itr++)
	{
		...
		//check to see if our filter is present
		if( strstr( (*itr)->message.c_str(), safe_keyword.c_str() ) == NULL )
			continue;
		...
		if( del_ticket == 1 )
		{
			objmgr.RemoveGMTicket((*itr)->guid,cplr->GetGUID());
			if( plr->IsInWorld() )
			{
				// Notify player about removing ticket
				sStackWorldPacket( data, SMSG_GMTICKET_DELETETICKET, 4);
				data << uint32(9);
		...
		if( teleport == 1 )
			plr->SafeTeleport( cplr->GetMapMgr(), cplr->GetPosition() );
	}
...
bool ChatHandler::HandleGMTicketGetByIdCommand(const char* args, WorldSession *m_session)
{
	...
	if(ticket == NULL || ticket->deleted_by)
	{
...
bool ChatHandler::HandleGMTicketRemoveByIdCommand(const char* args, WorldSession *m_session)
{
	...
	if( m_session->GetPlayer() && m_session->GetPlayer()->m_GM_statistics )
		m_session->GetPlayer()->m_GM_statistics->tickets_deleted++;
#endif
	Player *cplr = m_session->GetPlayer();
	Channel *chn = channelmgr.GetChannel(sWorld.getGmClientChannel().c_str(), cplr);
	if(!chn)
	...
	objmgr.RemoveGMTicket(ticket->guid,cplr->GetGUID());
