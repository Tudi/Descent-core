...
bool ChatHandler::HandleWPAddCommand(const char* args, WorldSession *m_session)
{
	...
	if(p->waypointunit != NULL)
	{
		...
		pCreature = SafeCreatureCast(ai->GetUnit());
		if( !pCreature || pCreature->IsCreature() == false || pCreature->IsPet() == true )
		{
			SystemMessage(m_session, "Invalid Creature, please select another one.");
			return true;
		}
	...
	if( ai->m_waypoints == NULL )
	{
		ai->m_waypoints = new WayPointVect(1);
		ai->m_custom_waypoint_map = ai->m_waypoints;
	}
	WayPoint* last_wp = ai->m_waypoints->GetValue( ai->m_waypoints->GetMaxSize() - 1 );
	uint32 next_unused_id;
	if( last_wp )
		next_unused_id = last_wp->id + 1;
	else next_unused_id = 1;
	wp->id = next_unused_id;
	wp->x = p->GetPositionX();
	wp->y = p->GetPositionY();
	wp->z = p->GetPositionZ();
...
bool ChatHandler::HandleWaypointsUpdatePos(const char* args, WorldSession *m_session)
{
	uint64 guid = m_session->GetPlayer()->GetSelection();
	if (guid == 0)
	{
	...
	if(wpid)
	{
		...
		SystemMessage(m_session, "Waypoint %u updated.", wpid);
	}
	else
	{
		SystemMessage(m_session, "Invalid Waypoint.");
...
bool ChatHandler::HandleNpcSelectCommand(const char * args, WorldSession * m_session)
{
	...
	for(itr = plr->GetInRangeSetBegin( AutoLock ); itr != plr->GetInRangeSetEnd(); ++itr)
	{
		if( (dist2 = plr->GetDistance2dSq(*itr)) < dist && (*itr)->GetTypeId() == TYPEID_UNIT )
		{
			un = (SafeCreatureCast(*itr));
			dist = dist2;
		}
	}
