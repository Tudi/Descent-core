...
#define CELL_EVENT_BASESHIFT 400 //just make sure we do not have a define like this
//1000 is greater then _sizeY so it is ok
#define MAKE_CELL_EVENT(x,y) ( ((x) * 1000) + CELL_EVENT_BASESHIFT + y )
#define DECODE_CELL_EVENT(dest_x, dest_y, ev) (dest_x) = ((ev-CELL_EVENT_BASESHIFT)/1000); (dest_y) = ((ev-CELL_EVENT_BASESHIFT)%1000);
...
public:
	...
	void LoadObjects(CellSpawnsVect * sp);
	...
	bool CanUnload();			//cells that contain objects that cannot go idle can not unload
	...
	uint32		ObjectCountToReserveOnEnter;
private:
	uint16		_x,_y;
	ObjectSet	_objects;
