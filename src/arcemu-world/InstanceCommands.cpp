...
{
	if( type & INSTANCE_FLAG_STATIC_MAP )
		return "Continent";
	else if( type & INSTANCE_FLAG_PVP )
		return "PvP";
	else if( ( type & ( INSTANCE_FLAG_DUNGEON_5_MEN | INSTANCE_FLAG_DUNGEON_5_MEN_HEROIC ) ) != 0
		&& ( type & ( INSTANCE_FLAG_RAID_10_MEN | INSTANCE_FLAG_RAID_10_MEN_HEROIC | INSTANCE_FLAG_RAID_25_MEN | INSTANCE_FLAG_RAID_25_MEN_HEROIC ) ) == 0	)
		return "Dungeon";
	else if( ( type & ( INSTANCE_FLAG_DUNGEON_5_MEN | INSTANCE_FLAG_DUNGEON_5_MEN_HEROIC ) ) == 0
		&& ( type & ( INSTANCE_FLAG_RAID_10_MEN | INSTANCE_FLAG_RAID_10_MEN_HEROIC | INSTANCE_FLAG_RAID_25_MEN | INSTANCE_FLAG_RAID_25_MEN_HEROIC ) ) != 0	)
		return "Raid";
...
bool ChatHandler::HandleGetInstanceInfoCommand(const char* args, WorldSession *m_session)
{
	...
	if(instance->m_mapInfo != NULL)
	{
		ss << "Type:" << MSG_COLOR_CYAN << GetDifficultyString(instance->instance_difficulty) << "|r)";
		ss << "\n";
	}
	...
	else if(!instance->m_mapMgr->HasPlayers())
	{
		...
		if(instance->m_mapMgr->InactiveMoveTime && instance->m_mapMgr->GetMapInfo()->type_flags != INSTANCE_FLAG_STATIC_MAP )
...
bool ChatHandler::HandleShowInstancesCommand(const char* args, WorldSession* m_session)
{
	...
	for(int difficulty=0; difficulty<NUM_INSTANCE_MODES; difficulty++)
	{
		for(PlayerInstanceMap::iterator itr = plr->m_playerInfo->savedInstanceIds[difficulty].begin(); itr != plr->m_playerInfo->savedInstanceIds[difficulty].end(); ++itr)
		{
			...
			{
				ss << " [" << GetDifficultyString(pInstance->instance_difficulty) << "]";
				ss << " - ";
