...
bool ChatHandler::HandleInvisibleCommand(const char *args, WorldSession *m_session)
{
	...
	if(pChar->IsGMInvisible())
	{
		pChar->RemGMInvisible();
...
bool ChatHandler::HandleDeleteCommand(const char* args, WorldSession *m_session)
{
	...
	if(unit->IsInWorld())
	{
		if(unit->m_spawn)
		{
			...
			uint32 cellx=CellHandler<MapMgr>::GetPosX(unit->m_spawn->x);
			uint32 celly=CellHandler<MapMgr>::GetPosY(unit->m_spawn->y);
			if(cellx <= _sizeX && celly <= _sizeY)
			{
				CellSpawnsVect * sp = unit->GetMapMgr()->GetBaseMap()->GetSpawnsListAndCreate(cellx, celly);
				uint32 max_size = sp->CreatureSpawns.GetMaxSize();
				for(uint32 ti=0;ti<max_size;ti++)
					if( sp->CreatureSpawns.GetValue( ti ) == unit->m_spawn )
					{
						sp->CreatureSpawns.DeleteValueAtIndex( ti );
						found_spawn = true;
						break; 
					}
				delete unit->m_spawn;
	...
	sGarbageCollection.AddObject( unit );
	unit = NULL;
...
bool ChatHandler::HandleMorphCommand(const char* args, WorldSession *m_session)
{
	...
	uint32 model_id;
	if(sscanf(args, "%u", &model_id) == 0)
	{
		RedSystemMessage(m_session, "Usage : .npc_morph <model_id>");
		return true;
	}
	...
	Unit *unit = m_session->GetPlayer()->GetMapMgr()->GetUnit(GET_LOWGUID_PART(guid));
	if(!unit)
	{
		SystemMessage(m_session, "You should select something to morph.");
		return true;
	}

	unit->SetUInt32Value( UNIT_FIELD_DISPLAYID, model_id);
	m_session->GetPlayer()->BroadcastMessage("Morphed target display to : %d",model_id);
...
bool ChatHandler::HandleItemCommand(const char* args, WorldSession *m_session)
{
	...
	{
		std::stringstream ss;
		ss << "INSERT INTO vendors VALUES ('" << pCreature->GetUInt32Value(OBJECT_FIELD_ENTRY) << "', '" << item << "', '" << amount << "', 0, 0, " << costid << ",1 )" << '\0';
...
bool ChatHandler::HandleKillCommand(const char *args, WorldSession *m_session)
{
	...
	{
	case TYPEID_PLAYER:
		sGMLog.writefromsession(m_session, "used kill command on PLAYER %s", SafePlayerCast( target )->GetName() );
		break;

	case TYPEID_UNIT:
		sGMLog.writefromsession( m_session, "used kill command on CREATURE %u [%s], sqlid %u%s", SafeCreatureCast( target )->GetEntry(), SafeCreatureCast( target )->GetCreatureInfo() ? SafeCreatureCast( target )->GetCreatureInfo()->Name : "unknown", SafeCreatureCast( target )->GetSQL_id(), m_session->GetPlayer()->InGroup() ? ", in group" : "" );
		break;
	}
	...
	if(target->IsPlayer())
	{
		Player * plr = SafePlayerCast(target);
		plr->SetUInt32Value(UNIT_FIELD_HEALTH, 1);
		m_session->GetPlayer()->DealDamage(plr, 666,0);
...
bool ChatHandler::HandleCastSpellCommand(const char* args, WorldSession *m_session)
{
	...
	if(!sp)
	{
		...
		SpellPool.PooledDelete( sp, __FILE__, __LINE__ );
		return false;
	}
	...
	switch ( target->GetTypeId() )
	{
		case TYPEID_PLAYER:
			if ( caster != target )
				sGMLog.writefromsession( m_session, "casted spell %d on PLAYER %s", spellid, SafePlayerCast( target )->GetName() );
			break;
		case TYPEID_UNIT:
			sGMLog.writefromsession( m_session, "casted spell %d on CREATURE %u [%s], sqlid %u", spellid, SafeCreatureCast( target )->GetEntry(), SafeCreatureCast( target )->GetCreatureInfo() ? SafeCreatureCast( target )->GetCreatureInfo()->Name : "unknown", SafeCreatureCast( target )->GetSQL_id() );
			break;
	}
...
bool ChatHandler::HandleTargetCastCommand(const char* args, WorldSession *m_session)
{
	Unit *target = getSelectedChar(m_session, false);
	if(!target)
		target = getSelectedCreature(m_session, false);
	...
	BlueSystemMessage(m_session, "Target is casting spell %d on us", spellId);
	target->CastSpell( m_session->GetPlayer()->GetGUID(), spellId, true );
	return true;
}

bool ChatHandler::HandleForceCastCommand(const char* args, WorldSession *m_session)
{
	Unit *target = getSelectedChar(m_session, false);
	if(!target)
		target = getSelectedCreature(m_session, false);
	...
	BlueSystemMessage(m_session, "We are casting spell %d on Target", spellId);

	Player *plr = m_session->GetPlayer();
//	m_session->GetPlayer()->CastSpell( target, spellId, true );

	SpellEntry *spe = dbcSpell.LookupEntryForced( spellId );
	Spell *sp = SpellPool.PooledNew( __FILE__, __LINE__ );
	sp->Init( plr, spe, true, NULL);
	SpellCastTargets tgt( target->GetGUID() );
	tgt.m_unitTarget = target->GetGUID();
	sp->m_targetUnits[0].push_back( target->GetGUID() );
	sp->m_targetUnits[1].push_back( target->GetGUID() );
	sp->m_targetUnits[2].push_back( target->GetGUID() );
	sp->prepare(&tgt);
...
bool ChatHandler::HandleCastSelfCommand(const char* args, WorldSession *m_session)
{
	...
	Spell *sp = SpellPool.PooledNew( __FILE__, __LINE__ );
	sp->Init(target, spellentry, true, NULL);
	if(!sp)
	{
		RedSystemMessage(m_session, "Spell failed creation!");
		SpellPool.PooledDelete( sp, __FILE__, __LINE__ );
	...
	switch ( target->GetTypeId() )
	{
		case TYPEID_PLAYER:
			if ( m_session->GetPlayer() != target )
				sGMLog.writefromsession( m_session, "used castself with spell %d on PLAYER %s", spellid, SafePlayerCast( target )->GetName() );
			break;
		case TYPEID_UNIT:
			sGMLog.writefromsession( m_session, "used castself with spell %d on CREATURE %u [%s], sqlid %u", spellid, SafeCreatureCast( target )->GetEntry(), SafeCreatureCast( target )->GetCreatureInfo() ? SafeCreatureCast( target )->GetCreatureInfo()->Name : "unknown", SafeCreatureCast( target )->GetSQL_id() );
			break;
	}
...
bool ChatHandler::HandleGOSelect(const char *args, WorldSession *m_session)
{
	...
	InRangeSetRecProt::iterator Itr = m_session->GetPlayer()->GetInRangeSetBegin( AutoLock );
	InRangeSetRecProt::iterator Itr2 = m_session->GetPlayer()->GetInRangeSetEnd();
	float cDist = 9999.0f;
	float nDist = 0.0f;
	bool bUseNext = false;
		...
		if(args[0] == '1')
		{
			...
			for(;;Itr++)
			{
				if(Itr == Itr2 && GObj == NULL && bUseNext)
					Itr = m_session->GetPlayer()->GetInRangeSetBegin( AutoLock );
				else if(Itr == Itr2)
					break;
...
bool ChatHandler::HandleGODelete(const char *args, WorldSession *m_session)
{
	...
	if(GObj->m_spawn != 0 && GObj->m_spawn->entry == GObj->GetEntry())
	{
		...
		bool found_spawn = false;
		uint32 cellx=CellHandler<MapMgr>::GetPosX(GObj->m_spawn->x);
		uint32 celly=CellHandler<MapMgr>::GetPosY(GObj->m_spawn->y);

		if(cellx < _sizeX && celly < _sizeY)
		{
			//m_session->GetPlayer()->GetMapMgr()->GetBaseMap()->GetSpawnsListAndCreate(cellx,celly)->GOSpawns.erase(GObj->m_spawn);
			CellSpawnsVect * sp = GObj->GetMapMgr()->GetBaseMap()->GetSpawnsListAndCreate(cellx, celly);
			uint32 max_size = sp->GOSpawns.GetMaxSize();
			for(uint32 ti=0;ti<max_size;ti++)
				if( sp->GOSpawns.GetValue( ti ) == GObj->m_spawn )
				{
					sp->GOSpawns.DeleteValueAtIndex( ti );
					found_spawn = true;
					break; 
				}
			if( GObj->m_spawn )
				ASSERT( found_spawn );
			//should not be respawning tough
			MapCell *m_respawnCell = m_session->GetPlayer()->GetMapMgr()->GetCellByCoords( GObj->m_spawn->x, GObj->m_spawn->y );
			delete GObj->m_spawn;
			GObj->m_noRespawn = true;
			GObj->m_spawn = NULL;
			if( m_respawnCell )
			{
				ObjectSet::iterator itr = m_respawnCell->_respawnObjects.find( GObj );
				if( itr != m_respawnCell->_respawnObjects.end() )
					m_respawnCell->_respawnObjects.erase( itr );
			}
		}
	}
	sEventMgr.RemoveEvents( GObj );
	GObj->RemoveFromWorld( true );
//	sGarbageCollection.AddObject( GObj );	//leaking memory ? This is crash reason in 88% of server crash cases :S
...
bool ChatHandler::HandleGOSpawn(const char *args, WorldSession *m_session)
{
	...
	gs->parent_rot2 = go->GetFloatValue(GAMEOBJECT_PARENTROTATION_1);
...
bool ChatHandler::HandleGOInfo(const char *args, WorldSession *m_session)
{
	...
	SystemMessage(m_session, "%s State:%s%u",MSG_COLOR_GREEN,MSG_COLOR_LIGHTBLUE,GObj->GetByte(GAMEOBJECT_BYTES_1, GAMEOBJECT_BYTES_STATE));
	...
	char gotypetxt[50];
	switch( GObj->GetByte(GAMEOBJECT_BYTES_1, GAMEOBJECT_BYTES_TYPEID) )
	{
	...
	SystemMessage(m_session, "%s Type:%s%u -- %s",MSG_COLOR_GREEN,MSG_COLOR_LIGHTBLUE,GObj->GetByte(GAMEOBJECT_BYTES_1, GAMEOBJECT_BYTES_TYPEID),gotypetxt);
	...
	SystemMessage(m_session, "%s Size:%s%f",MSG_COLOR_GREEN,MSG_COLOR_LIGHTBLUE,GObj->GetFloatValue(OBJECT_FIELD_SCALE_X));
	SystemMessage(m_session, "%s X:%s%f",MSG_COLOR_GREEN,MSG_COLOR_LIGHTBLUE,GObj->GetPositionX());
	SystemMessage(m_session, "%s Y:%s%f",MSG_COLOR_GREEN,MSG_COLOR_LIGHTBLUE,GObj->GetPositionY());
	SystemMessage(m_session, "%s Z:%s%f",MSG_COLOR_GREEN,MSG_COLOR_LIGHTBLUE,GObj->GetPositionZ());
	SystemMessage(m_session, "%s O:%s%f",MSG_COLOR_GREEN,MSG_COLOR_LIGHTBLUE,GObj->GetOrientation());
	SystemMessage(m_session, "%s Rotation X:%s%f",MSG_COLOR_GREEN,MSG_COLOR_LIGHTBLUE,GObj->GetFloatValue(GAMEOBJECT_PARENTROTATION));
	SystemMessage(m_session, "%s Rotation Y:%s%f",MSG_COLOR_GREEN,MSG_COLOR_LIGHTBLUE,GObj->GetFloatValue(GAMEOBJECT_PARENTROTATION_1));
	SystemMessage(m_session, "%s Rotation O1:%s%f",MSG_COLOR_GREEN,MSG_COLOR_LIGHTBLUE,GObj->GetFloatValue(GAMEOBJECT_PARENTROTATION_2));
	SystemMessage(m_session, "%s Rotation O2:%s%f",MSG_COLOR_GREEN,MSG_COLOR_LIGHTBLUE,GObj->GetFloatValue(GAMEOBJECT_PARENTROTATION_3));
...
//	sp->spelltargetType = atoi(spelltargetType);
	sp->min_hp_pct = (uint32)atof(floatMisc1);
	sp->max_hp_pct = (uint32)atof(Misc2);
...
bool ChatHandler::HandleGOAnimProgress(const char * args, WorldSession * m_session)
{
	...
	uint32 ap = atol(args);
	GObj->SetByte(GAMEOBJECT_BYTES_1, GAMEOBJECT_BYTES_ANIMPROGRESS, ap);
...
bool ChatHandler::HandleNPCEquipOneCommand(const char * args, WorldSession * m_session)
{
	...
	if( !SelectedCreature || !SelectedCreature->GetCreatureInfo() || !SelectedCreature->GetProto() || !SelectedCreature->m_spawn )
	{
...
bool ChatHandler::HandleGetLootChance(const char * args, WorldSession * m_session)
{
	if(!args)
		return false;

	uint32 ItemID = atol(args);
	ItemPrototype *ip = ItemPrototypeStorage.LookupEntry( ItemID );
	if( ip == NULL )
	{
		m_session->GetPlayer()->BroadcastMessage( "Could not find item to seach for" );
		return false;
	}
	bool FoundAny = false;
	m_session->GetPlayer()->BroadcastMessage( "Searching for item : %s", ip->Name1 );
	std::map<uint32, SimpleVect< SimpleVect<LootStoreStruct*>* >* >::iterator entryitr;
	for( entryitr = objmgr.m_creature_loot_kill.begin(); entryitr != objmgr.m_creature_loot_kill.end(); entryitr++ )
	{
		uint32 Entry = entryitr->first;
		SimpleVect< SimpleVect<LootStoreStruct*>* > *ListOfLists = entryitr->second;
		if( ListOfLists == NULL )
			continue;
		for( uint32 i=0;i<ListOfLists->GetMaxSize(); i++ )
		{
			SimpleVect<LootStoreStruct*> *GroupList = ListOfLists->GetValue( i );
			if( GroupList == NULL )
				continue;
			for( uint32 j=0;j<GroupList->GetMaxSize(); j++ )
			{
				LootStoreStruct *loot = GroupList->GetValue( j );
				if( loot == NULL )
					continue;
				if( loot->ItemEntry == ItemID )
				{
					CreatureInfo *ci = CreatureNameStorage.LookupEntry( loot->entry );
					if( ci )
					{
						m_session->GetPlayer()->BroadcastMessage( "On creature '%s' %d with chance %f. Creature loot list size %d", ci->Name, ci->Id, loot->DropChance, GroupList->GetMaxSize() );
						FoundAny = true;
					}
				}
			}
	...
	if( FoundAny == false )
		m_session->GetPlayer()->BroadcastMessage( "Could not find any creatures that drop this item");
	return true;
}
