...
enum WeatherTypes
{
	 ...
	 //there are like 35 values here actually
     WEATHER_TYPE_RAIN2		        = 21, 
     WEATHER_TYPE_SNOW2		        = 83, 
     WEATHER_TYPE_LEAFBROWN	        = 90, 
     WEATHER_TYPE_RAIN3		        = 91, 
     WEATHER_TYPE_SNOW3		        = 96, 
     WEATHER_TYPE_SNOW4		        = 97, 
     WEATHER_TYPE_SNOW5		        = 98, 
     WEATHER_TYPE_RAIN4		        = 99, 
     WEATHER_TYPE_RAIN_BLOOD        = 100, 
     WEATHER_TYPE_RAIN5		        = 102, 
     WEATHER_TYPE_SNOW_MIST	        = 104, 
     WEATHER_TYPE_RAIN_BLOOD2        = 108, 
};
...
{
/*
- not even sure this is the right packet. If so then struct changed
{SERVER} Packet: (0x777F) UNKNOWN PacketSize = 9 TimeStamp = 49480069
54 00 00 00 9A 99 99 3E 00 
*/
...
//		*data << Effect << float(0) << uint32(0) << uint8(0);		
		*data << Effect << float(0) << uint8(0);		
	else
//		*data << Effect << Density << GetSound(Effect,Density) << uint8(0) ;
		*data << Effect << Density << uint8(0) ;
...
void WeatherMgr::SendWeather(Player *plr)  //Update weather when player has changed zone (WorldSession::HandleZoneUpdateOpcode)
{
	...
	{
		WorldPacket data(SMSG_WEATHER, 12);
