...
void WordFilter::Load(const char * szTableName)
{
	...
	do 
	{
		...
		pMatch->szMatch = (strlen(pResult->Fetch()[0].GetString()) > 1) ? strdup_local(pResult->Fetch()[0].GetString()) : NULL;
		...
		pMatch->szIgnoreMatch = (strlen(pResult->Fetch()[1].GetString()) > 1) ? strdup_local(pResult->Fetch()[1].GetString()) : NULL;
