...
void RemoteConsole::Write(const char * Format, ...)
{
	...
	va_start(ap, Format);
	vsnprintf(obuf, 16000, Format, ap);
	va_end(ap);
...
void HandleConsoleInput(BaseConsole * pConsole, const char * szInput)
{
	static ConsoleCommand Commands[] = {
		...
		{ &HandleMemInfoCommand, "meminfo", "none", "Gives server object counts info." },
...
//		{ &HandleOnlinePlayersCommand, "online", "none", "Shows online players." },
		...
		{ &HandleModPlayerLevel, "levelplr", "<name> <levels>", "ads levels to a Player" },
		{ &HandleModPlayerTalents, "addtalentpointsplr", "<name> <points>", "add or take talent points to/from a Player" },
		{ &HandleModPlayerProfessions, "addprimaryprofession", "<name> <skill>", "Learn the player a new profession if it is posssible. Or max out existing one" },
		{ &HandleModPlayerProfessionsSecondary, "addsecondaryprofession", "<name> <skill>", "Learn the player a new profession" },
		{ &HandlePlayerSave, "saveplr", "<name>", "save the state of an online player" },
		{ &HandlePlayerAddItem, "additemplr", "<name> <itemid> <count>", "Add N count of the same item" },
		{ &HandleModPlayerGold, "addgoldplr", "<name> <copper>", "Add N copper to player" },
		{ &HandlePlayerAddspell, "learnspellplr", "<name> <spellid>", "Learns a spell to the player" },
		{ &HandlePlayerCastSpell, "castspellplr", "<name> <spellid>", "Casts a spell on the player" },
		{ &HandlePlayerAddTitle, "setTitle", "<name> <titleID>", "Set title on a player" },
		{ NULL, NULL, NULL, NULL },
	};
