...
bool ChatHandler::HandleInfoCommand(const char* args, WorldSession *m_session)
{
...
#endif
	GreenSystemMessage(m_session, "Server Revision: |r%s%s r%u/%s-%s-%s %s%s", MSG_COLOR_WHITE,REPACK,
		BUILD_REVISION, CONFIG, PLATFORM_TEXT, ARCH, MSG_COLOR_LIGHTBLUE, REPACK_WEBSITE);
	...
	GreenSystemMessage(m_session, "Average Client Latency: |r%.3fms", (float)((float)avg / (float)count));
	GreenSystemMessage(m_session, "Average Server Latency: |r%.3fms", AvgLatency);
	GreenSystemMessage(m_session, "Instances: |r%d", ActiveMapCount);
	GreenSystemMessage(m_session, "Creatures: |r%d", ActiveCreatureCount);
	GreenSystemMessage(m_session, "GameObjects: |r%d", ActiveGamobjects);
	GreenSystemMessage(m_session, "Corpses: |r%d", ActiveCorpses);
	GreenSystemMessage(m_session, "Delayed Deletes: |r%d", sGarbageCollection.size() );
	GreenSystemMessage(m_session, "Pooled memory MB: |r%.4f / %.4f ", ( ItemPool.GetPoolSizeMB() + AuraPool.GetPoolSizeMB() + SpellPool.GetPoolSizeMB() ) / 1024, ( ItemPool.GetPoolUnusedSizeMB() + AuraPool.GetPoolUnusedSizeMB() + SpellPool.GetPoolUnusedSizeMB() ) / 1024 );
