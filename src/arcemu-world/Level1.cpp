...
//	int32 arg_nr = atoi( args );
//	int64 *icon_nr = m_session->GetPlayer()->GetCreateIn64Extension( "GMIcon" );
//	*icon_nr = arg_nr;
...
//	AreaTable * at = dbcArea.LookupEntry( obj->GetMapMgr()->GetAreaID(obj->GetPositionX(), obj->GetPositionY()) );
	AreaTable * at = dbcArea.LookupEntry( sTerrainMgr.GetAreaID( obj->GetMapId(), obj->GetPositionX(), obj->GetPositionY(), obj->GetPositionZ() ) );
	if(!at) 
		return true;

	char buf[700];
	snprintf((char*)buf, 700, "|cff00ff00Current Position: |cffffffffMap: |cff00ff00%d |cffffffffZone: |cff00ff00%u |cffffffffArea: |cff00ff00%d |cffffffffX: |cff00ff00%f |cffffffffY: |cff00ff00%f |cffffffffZ: |cff00ff00%f |cffffffffOrientation: |cff00ff00%f|r",
		(unsigned int)obj->GetMapId(), at->ZoneId, at->AreaId, obj->GetPositionX(), obj->GetPositionY(), obj->GetPositionZ(), obj->GetOrientation());
	SystemMessage(m_session, buf);
	if( obj == m_session->GetPlayer() )
	{
		snprintf((char*)buf, 700, "|cff00ff00Current Stored values: |cffffffffMap: |cff00ff00%d |cffffffffZone: |cff00ff00%u |cffffffffArea: |cff00ff00%d |r",
		(unsigned int)obj->GetMapId(), m_session->GetPlayer()->GetZoneId(), m_session->GetPlayer()->GetAreaID());
		SystemMessage(m_session, buf);
	}
...
bool ChatHandler::HandleKickHomeCommand(const char* args, WorldSession *m_session)
{
	...
	if(!pname)
	{
		...
		if (guid != 0)
		{
			...
			if( chr == NULL)
			{
				SystemMessage(m_session, "You should select a character to kick.");
				return true;
			}
		}
	}
	else
		chr = objmgr.GetPlayer((const char*)pname, false);
	if( chr && chr->GetMapMgr() )
	{
		//some noobs miss bind location so we force them in main city
		if( chr->GetBindPositionX() == 0 ) 
		{
			if( chr->GetTeam() == 0 )
				chr->SetBindPoint( -9100,406,93,0,0);
			else
				chr->SetBindPoint( 1371,-4370,27,1,0);
		}

		sEventMgr.AddEvent(chr,&Player::EventTeleport,chr->GetBindMapId(),chr->GetBindPositionX(),chr->GetBindPositionY(),
			chr->GetBindPositionZ(),EVENT_PLAYER_TELEPORT,50,1,EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
...
bool ChatHandler::HandleSummonCommand(const char* args, WorldSession *m_session)
{
	...
	if( m_session->GetPlayer() && m_session->GetPlayer()->m_GM_statistics )
		m_session->GetPlayer()->m_GM_statistics->summons_made++;
#endif
	...
	if (chr)
	{
		...
		if(!m_session->GetPlayer()->IsGMInvisible())
		{
		...
		{
			sEventMgr.AddEvent(chr,&Player::EventPortToGM,(Unit*)plr,0,1,1,EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
		}
	}
	else
	{
...
#if GM_STATISTICS_UPDATE_INTERVAL > 0 
	if( m_session->GetPlayer() && m_session->GetPlayer()->m_GM_statistics )
		m_session->GetPlayer()->m_GM_statistics->appear_made++;
#endif
	Player *chr = objmgr.GetPlayer(args, false);
	if (chr)
	{
...
bool ChatHandler::HandleGetSkillLevelCommand(const char *args, WorldSession *m_session)
{
	...
	skilllineentry * Skille = dbcSkillLine.LookupEntryForced(skill);
	if(Skille == NULL)
	{
    ...
    if (!plr->_HasSkillLine(skill))
    {
        BlueSystemMessage(m_session, "Player does not have %u skill.", skill);
        return false;
    }
...
bool ChatHandler::HandleGetSkillsInfoCommand(const char *args, WorldSession *m_session)
{
	...
	for(itr = plr->m_skills.begin(); itr != plr->m_skills.end(); itr = itr->Next() )
		SystemMessage(m_session, "Skill %u %s %u/%u (bonus %u)", itr->data->Skill->id, itr->data->Skill->Name ,itr->data->CurrentValue, itr->data->MaximumValue,itr->data->BonusValue);
...
//	WorldPacket data;
	...
	int64 newgold = chr->GetGold( ) + total;
