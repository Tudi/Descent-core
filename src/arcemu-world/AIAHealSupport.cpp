...
char	bored_texts[BOREDOM_TEXTCOUNT][200]=
{
	...
	"If you stare at the sun a whole day you will see your future",
...
float GetSpellEficiencyFactor(SpellEntry *sp)
{
	...
	return (float)(sp->eff[0].EffectBasePoints) / ( GetCastTime( sd ) * 2 + sp->GetManaCost() + 1) ;
}*/
...
//    SetFollowDistance( 10.0f );
	...
	m_Unit->SetMaxPower( POWER_TYPE_MANA , (uint32)(m_Unit->GetUInt32Value( UNIT_FIELD_BASE_MANA ) * DifficultyLevel * CREATURE_STATS_SCALE_WITH_DIFFICULTY) );
	m_Unit->SetUInt32Value( UNIT_FIELD_HEALTH , m_Unit->GetUInt32Value( UNIT_FIELD_MAXHEALTH ) );
	m_Unit->SetPower( POWER_TYPE_MANA , m_Unit->GetMaxPower( POWER_TYPE_MANA ) );

	Unit *m_PetOwner = GetPetOwner();
...
//	if( m_Unit->GetPower( POWER_TYPE_MANA ) < sp->GetManaCost() )
//	if( m_Unit->GetPower( POWER_TYPE_MANA ) < sp->GetManaCost()*3 )
...
void AiAgentHealSupport::Update(uint32 p_time)
{
	...
	Unit *m_PetOwner;
	if( m_Unit->GetMapMgr() )
		m_PetOwner = m_Unit->GetMapMgr()->GetUnit( m_Unit->GetUInt64Value( UNIT_FIELD_CREATEDBY ) );
	else 
		m_PetOwner = NULL;

	if( !m_PetOwner || m_PetOwner->deleted != OBJ_AVAILABLE || m_Unit->GetMapMgr() != m_PetOwner->GetMapMgr() )
	{
		m_PetOwner = NULL;
		SafeCreatureCast(m_Unit)->Despawn(0,0);
		return;
	}
...
//printf("difficulty changed to %f \n",DifficultyLevel);
		...
		m_Unit->SetMaxPower( POWER_TYPE_MANA , (uint32)(m_Unit->GetUInt32Value( UNIT_FIELD_BASE_MANA ) * ( 1 + DifficultyLevel ) * CREATURE_STATS_SCALE_WITH_DIFFICULTY) );
		...
		m_Unit->SetPower( POWER_TYPE_MANA , (uint32)(m_Unit->GetPower( POWER_TYPE_MANA ) * ( 1 + DifficultyLevel ) * CREATURE_STATS_SCALE_WITH_DIFFICULTY) );
	}
	...
	if( CombatDifficultyLevel > DifficultyLevel )
		CombatDifficultyLevel = DifficultyLevel; 
...
//printf("master died, we are going to resurect him\n");
	...
	//if we are injured we should try to survive it
...
//printf("master is ok, we can try augment someone\n");
		...
		//try to augment raid if group size is large enough
		if( !m_castingSpell && m_PetOwner->IsPlayer() && SafePlayerCast(m_PetOwner)->GetGroup())
		{
			uint32 bonus_spell = 0;
			if( m_PetOwner->GetGroup()->MemberCount() >= 35 && m_PetOwner->HasAura( 73819 ) == 0 )
				bonus_spell = 73819;
			else if( m_PetOwner->GetGroup()->MemberCount() > 25 && m_PetOwner->HasAura( 73825 ) == 0 )
				bonus_spell = 73825;
			else if( m_PetOwner->GetGroup()->MemberCount() > 24 && m_PetOwner->HasAura( 23768 ) == 0 )
				bonus_spell = 23768;
			else if( m_PetOwner->GetGroup()->MemberCount() > 23 && m_PetOwner->HasAura( 23769 ) == 0 )
				bonus_spell = 23769;
			else if( m_PetOwner->GetGroup()->MemberCount() > 22 && m_PetOwner->HasAura( 23737 ) == 0 )
				bonus_spell = 23737;
			else if( m_PetOwner->GetGroup()->MemberCount() > 21 && m_PetOwner->HasAura( 23735 ) == 0 )
				bonus_spell = 23735;
			else if( m_PetOwner->GetGroup()->MemberCount() > 20 && m_PetOwner->HasAura( 23767 ) == 0 )
				bonus_spell = 23767;
			else if( m_PetOwner->GetGroup()->MemberCount() > 20 && m_PetOwner->HasAura( 23766 ) == 0 )
				bonus_spell = 23766;
			if( bonus_spell )
			{
				Player *powner = SafePlayerCast(m_PetOwner);
					...
					if( subgroup )
					{
						...
						for( GroupMembersSet::iterator itr = subgroup->GetGroupMembersBegin(); itr != subgroup->GetGroupMembersEnd(); ++itr )
						{
							if( (*itr)->m_loggedInPlayer && (*itr)->m_loggedInPlayer->isAlive()
								&& IsInrange( (*itr)->m_loggedInPlayer ,powner,range))
							{
								SpellEntry *spellInfo = dbcSpell.LookupEntry( bonus_spell );
								Spell *spell = SpellPool.PooledNew( __FILE__, __LINE__ );
								spell->Init((*itr)->m_loggedInPlayer, spellInfo ,true, NULL);
								spell->forced_duration = 5*60*1000;
								targets.m_unitTarget = (*itr)->m_loggedInPlayer->GetGUID();
...
//			m_Unit->ModPower( POWER_TYPE_MANA, -(int32)(m_castingSpell->sp->GetManaCost()) );
		}
...
//printf("we are far from owner, we should move closer , dist %f from %f \n",dist,(FollowDistance*FollowDistance));
		if ( dist > FollowDistance ) //if out of range
		{
			...
			float delta_x = m_PetOwner->GetPositionX();
			float delta_y = m_PetOwner->GetPositionY();
			float d = 3;

			MoveTo(delta_x+(d*(cosf(m_followAngle+m_PetOwner->GetOrientation()))),
				delta_y+(d*(sinf(m_followAngle+m_PetOwner->GetOrientation()))),
...
healagentspell* AiAgentHealSupport::PickSpellFromList(SpellList *spl,Unit *target)
{
	...
	for(itr=spl->begin();itr!=spl->end();itr++)
		if( CheckCanCast( (*itr)->sp, target ) && RandChance( (*itr)->chance_to_cast ) )
			return *itr;
	return NULL;
}
#endif