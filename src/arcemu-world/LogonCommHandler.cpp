...
LogonCommHandler::LogonCommHandler()
{
	...
	memset(&sql_passhash[0],0,20);
	memcpy(&sql_passhash[0], hash.GetDigest(), 20);
...
{
	if( ( sWorld.getIntRate( INTRATE_LOGON_TYPE ) & 1 ) == 0 )
	{
		return;
	}

    if (sMaster.m_ShuttingDown)
    {
		Log.Notice("LogonCommClient", "Tried to connect after shutdown started...");
        return;
    }
