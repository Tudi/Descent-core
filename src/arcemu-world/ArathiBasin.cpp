...
//		{ 180091,       180085,            180086,         180076,                180078 },			// MINE
		{ 180090,       180085,            180086,         180076,                180078 },			// MINE
...
void ArathiBasin::SpawnBuff(uint32 x)
{
	...
	if(m_buffs[x] == NULL)
	{
		m_buffs[x] = SpawnGameObject(chosen_buffid, m_mapMgr->GetMapId(), BuffCoordinates[x][0], BuffCoordinates[x][1], BuffCoordinates[x][2], BuffCoordinates[x][3], 0, 114, 1);
...
void ArathiBasin::CaptureControlPoint(uint32 Id, uint32 Team)
{
	...
	// update the map
	if( Id < AB_NUM_CONTROL_POINTS && Team < 2 )
	{
	...
	{
		// not first
		event_ModifyRepeatInterval(EVENT_AB_RESOURCES_UPDATE_TEAM_0+Team, ResourceUpdateIntervals[m_capturedBases[Team]]);
	}
}
...
void ArathiBasin::AssaultControlPoint(Player * pPlayer, uint32 Id)
{
	...
	if(m_basesOwnedBy[Id] != -1)
	{
		...
		if(m_capturedBases[Owner]==0)
			...
			this->event_ModifyRepeatInterval(EVENT_AB_RESOURCES_UPDATE_TEAM_0 + Owner, ResourceUpdateIntervals[m_capturedBases[Owner]]);
	}
