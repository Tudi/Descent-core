...
bool ChatHandler::HandleListArenaMatches(const char *args, WorldSession *m_session)
{
	m_session->GetPlayer()->BroadcastMessage("Listing active arena matches:");
	objmgr.ListArenaMatches( m_session->GetPlayer(), 0, NULL, 0 );
	return true;
}
...
bool ChatHandler::HandleSetWorldStateCommand(const char* args, WorldSession *m_session)
{
	...
	{
		if(sscanf(args,"%X %u", &id,&val)!=2)
			return false;
	}

	if(m_session->GetPlayer()->m_bg)
		m_session->GetPlayer()->m_bg->SetWorldState(id,val);
	else if(m_session->GetPlayer()->GetMapMgr() )
		m_session->GetPlayer()->GetMapMgr()->SetWorldState(id,val);
...
{
	uint32 type = atoi(args);
	BattlegroundManager.SendBattlefieldStatus(m_session->GetPlayer(), BF_STATUS_PACKET_QUEUE_PLAYER, type, 0 , 0, m_session->GetPlayer()->GetMapId(),0);
	return true;
}
